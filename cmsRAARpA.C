...
//----------------------------------------- jets (RpA)
  ...
  TH1 *phAxis = new TH1D("phAxis",";p_{T} [GeV/c];Nuclear Modification Factor",1,0.9,300);
  ...
  TH1 *phAxis_single = new TH1D("phAxis_single",";p_{T} [GeV/c];Nuclear Modification Factor",1,0,105);
  phAxis_single->GetYaxis()->CenterTitle(true);
  phAxis_single->GetYaxis()->SetTitleOffset(1.1);
  ...
  TH1 *phAxis_jet = new TH1D("phAxis_jet",";p_{T} [GeV/c];Nuclear Modification Factor",1,0,400);
  phAxis_jet->GetYaxis()->CenterTitle(true);
  phAxis_jet->GetYaxis()->SetTitleOffset(1.1);
  //phAxis_jet->GetYaxis()->CenterTitle(true);
  ...
  TLegend *leg2_jetUnc = new TLegend(0.088,0.629,0.605,0.685,NULL,"brNDC");
  leg2_jetUnc->SetBorderSize(0);
  leg2_jetUnc->SetLineColor(1);
  leg2_jetUnc->SetLineStyle(1);
  leg2_jetUnc->SetLineWidth(1);
  leg2_jetUnc->SetFillColor(19);
  leg2_jetUnc->SetFillStyle(0);

    TLegendEntry *entry2u_jet=leg2_jetUnc->AddEntry("raaheavy","Inclusive jet  R_{pA} Reference Unc.","f");
    entry2u_jet->SetFillColor(TColor::GetColor("#FFBF00"));
    entry2u_jet->SetFillStyle(1001);
    entry2u_jet->SetLineColor(1);
    entry2u_jet->SetLineStyle(1);
    entry2u_jet->SetLineWidth(1);
    entry2u_jet->SetMarkerStyle(25);
    entry2u_jet->SetMarkerColor(kBlack);
  ...
  TLegendEntry *entry2_jet;

    if(version > 5)
  {
    entry2_jet=leg2_jet->AddEntry("raaheavy","Inclusive jet  R_{pA}  (0-100%) |#eta_{CM}| < 0.5","lpf");
  ...
  entry2_jet=leg2_jet->AddEntry("raaq","Inclusive jet  R_{AA}  (0-5%)  |#eta| < 2","lpf");
...
//    if(version > 5) pgRaa_npjpsi->Draw("P z");
    ...
    if(version > 4)
    {
      ...
      TLine *line_jet = new TLine(0,1,400,1);
