...
#endif
void cmsRAARpA2Sys(int version = 7)
{
...
//----------------------------------------- charged hadrons (RpA)
  ...
  double raaSyst2_hp[33];

  double ptSystXlow_hp[33];
  double ptSystXhigh_hp[33];
  double ptSystXlow2_hp[33];
  double ptSystXhigh2_hp[33];

  TBox * pgSyst2Box_hp[33];
  ...
  while(!inData_hp.eof())
  {
    ...
    raaSyst2_hp[j]  = 0.1*raa;
    raaSyst_hp[j]  = sqrt( raaSyst_hp[j]*raaSyst_hp[j] - 0.1*0.1*raa*raa);

    ptSystXlow_hp[j] = (xx_high-xx_low)/2;
    ptSystXhigh_hp[j]= (xx_high-xx_low)/2;
    ptSystXlow2_hp[j] = raa- 0.1;
    ptSystXhigh2_hp[j]= raa+ 0.1;
    // cout<<"pT"<<ptBins_h[j]<<"\t raa= "<<raa_h[j]<<"\t syst= "<<raaStat_h[j]<<"\t stat ="<<raaSyst_h[j]<<"\t x_low= "<<ptSystXlow_h[j]<<"\t high= "<<ptSystXhigh_h[j]<<endl;
    pgSyst2Box_hp[j] = new TBox(ptBins_hp[j] - ptSystXhigh2_hp[j], raa - raaSyst2_hp[j],
                          ptBins_hp[j] + ptSystXhigh2_hp[j], raa + raaSyst2_hp[j]);
    pgSyst2Box_hp[j]->SetLineColor(kBlack);
    pgSyst2Box_hp[j]->SetLineWidth(2);
    pgSyst2Box_hp[j]->SetFillStyle(0);
  ...
  TGraphAsymmErrors *pgRaaSyst2_hp = new TGraphAsymmErrors(33, ptBins_hp, raa_hp, ptSystXlow2_hp,ptSystXhigh2_hp,raaSyst2_hp,raaSyst2_hp);
  ...
  //systm error ref
  pgRaaSyst2_hp->SetName("pgRaaSyst2_hp");
  pgRaaSyst2_hp->SetFillColor(TColor::GetColor("#ffff00"));
...
//    raaSyst_jp[j]  = sqrt( raa_stat*raa_stat + raa_stat2*raa_stat2)/100.* raa;
    raaSyst_jp[j]  = raa_stat2/100.* raa;
    raaSyst2_jp[j]  = raa_stat/100.* raa;

    ptSystXlow_jp[j] = (xx_high-xx_low)/2;
    ptSystXhigh_jp[j]= (xx_high-xx_low)/2;
    // cout<<"pT"<<ptBins_h[j]<<"\t raa= "<<raa_h[j]<<"\t syst= "<<raaStat_h[j]<<"\t stat ="<<raaSyst_h[j]<<"\t x_low= "<<ptSystXlow_h[j]<<"\t high= "<<ptSystXhigh_h[j]<<endl;
    pgSyst2Box_jp[j] = new TBox(ptBins_jp[j] -3.0, raa - raaSyst2_jp[j],
                          ptBins_jp[j] + 3.0, raa +raaSyst2_jp[j]);
    pgSyst2Box_jp[j]->SetLineColor(kBlack);
    pgSyst2Box_jp[j]->SetLineWidth(2);
    pgSyst2Box_jp[j]->SetFillStyle(0);
  ...
  TGraphAsymmErrors *pgRaaSyst_jp = new TGraphAsymmErrors(19, ptBins_jp, raa_jp, ptSystXlow_jp,ptSystXhigh_jp,raaSyst_jp,raaSyst_jp);
  pgRaa_jp->SetName("pgRaa_jp");
  pgRaa_jp->SetMarkerStyle(21);
  ...
  TH1 *phAxis = new TH1D("phAxis",";p_{T} [GeV];R_{AA} / R_{pA}",1,0.9,300);
  phAxis->GetYaxis()->CenterTitle(true);
  phAxis->GetYaxis()->SetTitleOffset(0.8);
  phAxis->SetMinimum(0.);
  ...
  TLegendEntry *entry=leg->AddEntry("hEtSIEIECorrected","CMS  *PRELIMINARY","");
  entry->SetLineWidth(1);
  entry=leg->AddEntry("hEtSIEIECorrected","PbPb #sqrt{s_{NN}} = 2.76 TeV  #int L dt = 129-150 #mub^{-1}","");
  entry->SetLineWidth(1);
  entry=leg->AddEntry("hEtSIEIECorrected","pPb #sqrt{s_{NN}} = 5.02 TeV  #int L dt = 29-35 nb^{-1}","");
  entry->SetLineWidth(1);
  ...
  TLegendEntry *entry_jet=leg_jet->AddEntry("raaq","*Inclusive jet  R_{AA}  (0-5%)  |#eta| < 2","lpf");
  ...
  TH1 *phAxis_single = new TH1D("phAxis_single",";p_{T} [GeV];R_{AA} / R_{pA}",1,0,105);
  phAxis_single->GetYaxis()->CenterTitle(true);
  phAxis_single->GetYaxis()->SetTitleOffset(0.95);
  ...
  TH1 *phAxis_jet = new TH1D("phAxis_jet",";p_{T} [GeV];",1,40,300);
  ...
  TLegend *leg2 = new TLegend(0.04,0.71,0.34,0.93,NULL,"brNDC");
  ...
  entry2=leg2->AddEntry("general","PbPb #sqrt{s_{NN}} = 2.76 TeV  #int L dt = 129-150 #mub^{-1}","");
  entry2->SetLineWidth(1);
  entry2=leg2->AddEntry("general","pPb #sqrt{s_{NN}} = 5.02 TeV  #int L dt = 29-35 nb^{-1}","");
  entry2->SetLineWidth(1);
  
  // jet leg2end
  TLegend *leg2_jet = new TLegend(0.08,0.63,0.64,0.71,NULL,"brNDC");
  ...
  TLegendEntry *entry2_jet=leg2_jet->AddEntry("raaq","*Inclusive jet  R_{AA}  (0-5%)  |#eta| < 2","lpf");
  ...
  if(version > 5)
  {
    entry2_jet=leg2_jet->AddEntry("raaheavy","*Inclusive jet  R_{pA}  |#eta| < 0.5","lpf");
  ...
  // B and h
  TLegend *leg2_q = new TLegend(0.14,0.81,0.7,0.92,NULL,"brNDC");
  ...
  if(version > 3)
  {
    entry2_q=leg2_q->AddEntry("raahp","*Charged particles  R_{pA}  |#eta| < 1","lpf");
    entry2_q->SetFillColor(TColor::GetColor("#ffff00"));
    entry2_q->SetFillStyle(1001);
    entry2_q->SetLineColor(1);
    entry2_q->SetLineStyle(1);
    entry2_q->SetLineWidth(1);
    entry2_q->SetMarkerStyle(23);
    entry2_q->SetMarkerSize(mymarkersize);
  }
...
//    entry2_q=leg2_q->AddEntry("raab","","");
  ...
  if(bDo2PadZoo)
  {
    ...
    pgRaa_hp->SetMarkerSize(mymarkersize);
...
//    if(version > 1) pgRaaSyst_z->Draw("2");
 ...
 //   if(version > 3) pgRaaSyst2_hp->Draw("E");
    for( int i=0; i<33; i++ ) pgSyst2Box_hp[i]->Draw();
...
//    if(version > 1) pgRaa_z->Draw("P z");
    ...
    if(version > 3) pgRaa_hp->Draw("P z");
...
//    if(version > 5) pgRaa_npjpsi->Draw("P z");
 ...
 //     if(version > 5) pgRaaSyst_bjet->Draw("2");
      ...
      for(int i=0;i<14;i++) pgSyst2Box_jp[i]->Draw();     
 
 //     if(version > 5) pgRaa_bjet->Draw("P z");
      if(version > 4) pgRaa_jet->Draw("P z");
      if(version > 4) pgRaa_jp->Draw("P z");
      
      
      TLine *line_jet = new TLine(40,1,300,1);
